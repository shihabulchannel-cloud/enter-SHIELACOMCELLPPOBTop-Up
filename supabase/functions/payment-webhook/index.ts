...
// Pure JS MD5 - no external dependency
...
async function processDigiflazzOrder(supabase: ReturnType<typeof createClient>, order: Record<string, unknown>) {
  ...
  try {
    const res = await fetch("https://api.digiflazz.com/v1/transaction", {
      ...
      body: JSON.stringify({ username: dfConfig.username, buyer_sku_code: product.provider_code, customer_no: order.target as string, ref_id: refId, sign, testing: dfConfig.testing === true }),
    });
    ...
    await supabase.from("sc_orders").update({
      ...
      digiflazz_ref: refId, digiflazz_sn: data.data?.sn || "", notes: data.data?.message || "", updated_at: new Date().toISOString(),
    }).eq("id", order.id);
  } catch (e) {
    console.error("Digiflazz error:", e);
    await supabase.from("sc_orders").update({ order_status: "processing", digiflazz_ref: refId, notes: "Sedang diproses", updated_at: new Date().toISOString() }).eq("id", order.id);
  }
}
  ...
  try {
    ...
    if (gateway === "tripay") {
      ...
      if (privateKey) {
        ...
        if (callbackSig && callbackSig !== expectedSig) return new Response("Signature invalid", { status: 400 });
      }
    ...
    if (!order || order.payment_status === "paid" || order.payment_status === "expired") return new Response("OK", { status: 200, headers: corsHeaders });
