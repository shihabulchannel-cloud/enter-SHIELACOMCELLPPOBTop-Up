...
import { transactionStore, type Transaction } from '@/lib/store';
import { cn } from '@/lib/utils';

const statusConfig = {
  pending: { label: 'Menunggu Pembayaran', icon: Clock, color: 'bg-yellow-500/10 text-yellow-600 border-yellow-500/30' },
...
export default function CekTransaksi() {
  ...
  const handleCheck = () => {
    ...
    setNotFound(false);
    setTimeout(() => {
      const tx = transactionStore.findByInvoice(invoiceId.trim());
      if (tx) {
        setResult(tx);
      } else {
        setNotFound(true);
        ...
        <section className="bg-hero-gradient pt-24 pb-16 md:pt-32 md:pb-20">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-white/60 max-w-md mx-auto text-sm md:text-base">
              Masukkan Invoice ID untuk melihat status transaksi Anda
            </p>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 max-w-2xl">
            <div className="bg-card border border-border/50 rounded-3xl p-6 md:p-8 shadow-card">
              <div className="flex gap-3 mb-6">
                <div className="flex-1 relative">
                  ...
                  <Input
                    ...
                    placeholder="Contoh: INV-20240101-001"
              ...
              <div className="flex items-start gap-2 text-muted-foreground text-sm bg-muted rounded-xl p-3">
                ...
                <span>Coba contoh: <button onClick={() => setInvoiceId('INV-20240101-001')} className="text-primary font-medium hover:underline">INV-20240101-001</button></span>
              </div>
              ...
              {notFound && !loading && (
                <div className="mt-8 text-center py-12 animate-scale-in">
                  ...
                  <p className="text-muted-foreground text-sm">Invoice ID tidak ditemukan. Pastikan ID yang Anda masukkan sudah benar.</p>
                </div>
              )}
                  ...
                  <div className="border border-border/50 rounded-2xl overflow-hidden">
                    ...
                    <div className="bg-muted px-5 py-4 flex items-center justify-between flex-wrap gap-3">
                      <div>
                        ...
                        <p className="font-bold text-foreground">{result.invoiceId}</p>
                      </div>
                      {(() => {
                        const cfg = statusConfig[result.status];
                        const Icon = cfg.icon;
                        return (
                          <Badge className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-xl border font-semibold text-sm', cfg.color)}>
                            <Icon className={cn('w-4 h-4', result.status === 'processing' && 'animate-spin')} />
                            {cfg.label}
                          </Badge>
                        );
                      })()}
                    ...
                    <div className="p-5 space-y-3">
                      {[
                        { label: 'Produk', value: result.product },
                        { label: 'Tujuan', value: result.destination },
                        { label: 'Nominal', value: `Rp ${result.amount.toLocaleString('id-ID')}` },
                        { label: 'Waktu', value: result.createdAt },
                      ].map(item => (
