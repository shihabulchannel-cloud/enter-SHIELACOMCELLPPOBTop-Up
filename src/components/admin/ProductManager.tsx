...
import { Plus, Trash2, Edit2, Check, X, Search, RefreshCw, Database, ToggleLeft, ToggleRight } from 'lucide-react';
    ...
    <div className={cn('bg-card border rounded-2xl p-3 flex items-center gap-3 group transition-all', product.active ? 'border-border' : 'border-border/30 opacity-60')}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge className="text-xs bg-muted text-muted-foreground border-border">{CATEGORY_LABELS[product.category_id] || product.category_id}</Badge>
          <Badge className="text-xs bg-muted text-muted-foreground border-border">{product.brand || '-'}</Badge>
        ...
        <div className="flex gap-3 mt-0.5 flex-wrap">
          ...
          <span className="text-xs text-emerald-600 font-medium">Profit: Rp {(product.sell_price - product.buy_price).toLocaleString('id-ID')}</span>
        </div>
      </div>
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
        <button onClick={() => onEdit(product)} className="p-1.5 rounded-lg hover:bg-primary/10 text-muted-foreground hover:text-primary transition-colors" title="Edit">
          <Edit2 className="w-4 h-4" />
        </button>
        <button onClick={() => onToggle(product)} className={cn('p-1.5 rounded-lg transition-colors text-muted-foreground', product.active ? 'hover:bg-red-500/10 hover:text-red-500' : 'hover:bg-green-500/10 hover:text-green-500')} title={product.active ? 'Nonaktifkan' : 'Aktifkan'}>
          {product.active ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
        </button>
        <button onClick={() => onDelete(product)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-muted-foreground hover:text-destructive transition-colors" title="Hapus">
...
function ProductForm({ initial, onSave, onCancel }: {
  ...
  return (
    <div className="bg-card border border-primary/30 rounded-2xl p-5 space-y-4 mb-4">
      <h3 className="font-semibold text-foreground text-sm">{initial?.id ? 'Edit Produk' : 'Tambah Produk Baru'}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input value={form.sku} onChange={e => f('sku')(e.target.value)} placeholder="TSEL10" className="rounded-xl h-9" disabled={!!initial?.id} />
        </div>
        <div>
        ...
        <div>
          <label className="text-xs font-medium text-muted-foreground mb-1 block">Harga Jual (Rp)</label>
      ...
      <div className="flex gap-2">
        <Button onClick={() => { if (form.name && form.sku) onSave(form); }} size="sm" className="bg-primary text-primary-foreground rounded-xl gap-1 btn-glow">
...
export default function ProductManager() {
  ...
  const handleSave = async (form: Partial<DbProduct>) => {
    if (editing) {
      await supabase.from('sc_products').update({ ...form, updated_at: new Date().toISOString() }).eq('id', editing.id);
      setEditing(null);
    } else {
      await supabase.from('sc_products').insert({ ...form, provider: 'manual', created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
      setAdding(false);
    }
    loadProducts();
  };
  ...
  const handleToggle = async (p: DbProduct) => {
    await supabase.from('sc_products').update({ active: !p.active, updated_at: new Date().toISOString() }).eq('id', p.id);
    loadProducts();
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground flex items-center gap-1.5">
            ...
            {activeCount} aktif dari {products.length} produk (dari database)
          </p>
        </div>
        <div className="flex gap-2">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-[150px]">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari nama/SKU/brand..." className="pl-9 rounded-xl h-9" />
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map(product => (
            <ProductRow key={product.id} product={product} onEdit={setEditing} onToggle={handleToggle} onDelete={handleDelete} />
          ))}
          {filtered.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              <Database className="w-8 h-8 mx-auto mb-2 opacity-30" />
              <p>{products.length === 0 ? 'Belum ada produk. Sync dari Digiflazz di menu Provider.' : 'Tidak ada produk yang cocok dengan filter.'}</p>
            </div>
          )}
        </div>
      )}
